...
@section('dashboard-contents')
    <div class="dashboard__content-wrap">
            ...
            <div class="col-12">
                <div class="dashboard__review-table">
                    <form id="contentForm" enctype="multipart/form-data"
                        action="{{ route('student.follow-up-action.update', $followUpAction->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="course_id" class="form-control" id="kursus">
                                ...
                                @foreach ($courses as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $followUpAction->course_id == $item->id ? 'selected' : '' }}>{{ $item->title }}
                                    </option>
                                @endforeach
                            </select>
                            @error('course_id')
                        ...
                        {{-- <div class="form-group">
                            ...
                            <iframe src="{{ route('student.follow-up-action.files', $followUpAction->file_path) }}"
                        ...
                        <div class="form-group">
                            ...
                            <div class="mt-2">
                                ...
                                <div id="fileInfoExisting">
                                    <div class="file-existing">
                                        <div class="d-flex align-items-center">
                                            <!-- Font Awesome Icon PDF -->
                                            <i class="fas fa-file-pdf fa-lg text-danger"></i>
                                            <span class="fw-bold">File Saat Ini</span>
                                        </div>
                                        <div class="file-actions">
                                            <!-- Preview Button -->
                                            <button type="button" id="previewExistingBtn" data-bs-toggle="modal"
                                                data-bs-target="#previewExistingModal" title="Preview">
                                                Pratinjau
                                            </button>
        ...
        <div class="modal fade" id="previewExistingModal" tabindex="-1" aria-labelledby="previewExistingModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-fullscreen">
                <div class="modal-content" style="border: 1px solid #dee2e6">
                    <div class="modal-header">
                        <h5 class="modal-title" id="previewExistingModalLabel">Pratinjau File PDF Saat ini</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body p-0">
                        <iframe src="{{ route('student.follow-up-action.files', $followUpAction->file_path) }}"
...
@push('scripts')
    ...
    <script>
        ...
        // Memasukkan Konten Lama ke Quill Editor
        const oldContent = {!! json_encode($followUpAction->summary) !!};
        ...
        document.getElementById('previewExistingBtn').addEventListener('click', () => {
            previewExistingModal.show();
        });
